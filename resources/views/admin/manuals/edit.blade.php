...
<div class="content-area py-1">
    <div class="container-fluid">
    	<div class="box box-block bg-white">
    	    <a href="{{ route('admin.manual.index') }}?type={{$manual->type}}" class="btn btn-default pull-right"><i class="fa fa-angle-left"></i> @lang('admin.back')</a>

			<h5 style="margin-bottom: 2em;">Update {{$heading}}</h5>

            <form class="form-horizontal" action="{{route('admin.manual.update', $manual->id )}}" method="POST" enctype="multipart/form-data" role="form">
            	{{csrf_field()}}
            	<input type="hidden" name="_method" value="PATCH">            	
            	<div class="form-group row">
					<label for="first_name" class="col-xs-12 col-form-label">Select Category</label>
					<div class="col-xs-10">
						{!! dropdown('category_id',$cats, $manual->category_id) !!}	
					</div>
				</div>
            	...
            	<div class="form-group row">
					...
					<div class="col-xs-10">
						<input class="form-control" type="text" value="{{ $manual->title }}" name="title" required id="title" placeholder="Part Name">
					</div>
				</div>
				...
				<div class="form-group row">
					...
					<div class="col-xs-10">
						<textarea name="description" id="myeditor">{{ $manual->description }}</textarea>
					</div>
				</div>				
				<div class="form-group row">
					<label for="mobile" class="col-xs-12 col-form-label">Manual</label>
					@if( $manual->picture != "" )
						<a href="{{ asset('/storage/'.$manual->picture) }}" target="_blank">View</a>
					@endif
					<div class="col-xs-6">
				...
				<div class="form-group row">
					...
					<div class="col-xs-10">
						...
						<a href="{{route('admin.manual.index')}}?type={{$manual->type}}" class="btn btn-default">@lang('admin.cancel')</a>
					</div>
				</div>
			</form>
		</div>
